...
def is_valid_label(label):
  """
  Return True if label is a valid Python/CGNS Label
  NB: cgns_keywords written in C++ and wrap with pybind11)
  """
...
def check_is_label(label):
  """
    Decorator to check that a function wait a node with a specific label
  """
...
def requestChildFromPredicate(parent, predicate, method=NodeParser.DEFAULT, depth=None):
  """ Return the first child of parent node matching a given predicate or None if nothing has been found """
...
def getChildFromPredicate(parent, predicate, default=None, method=NodeParser.DEFAULT, depth=None):
  """ Return the first child of parent node matching a given predicate or raise an exception error if nothing has been found """
...
def getChildrenFromPredicate(parent, predicate, method=NodeParser.DEFAULT, depth=None):
  """ Return the list of nodes matching a given predicate """
